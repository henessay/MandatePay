...
| On-chain mirror `MandatePolicy.sol` | ✅ | **Real** contract + Foundry tests; deploy pending (owner) |
...
- **Pending owner action:** deploy `MandatePolicy.sol` to a testnet and wire the on-chain check into
  the payout path.

```
MANDATE_POLICY_ADDRESS = <TODO: fill in after `forge create` — see docs/R1_RESOLUTION.md / README run section>
```
